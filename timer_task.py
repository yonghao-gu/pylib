...
'''
    ...
    def __init__(self, trigger_type, ti):
        ...
        self.m_now = time_op.start_time()  #设置为最早的一天
    ...
    def __recyle_can_triger(self):
        return time_op.second() - self.m_now > self.m_args[0]

    def __today_can_trigger(self):
        now = time_op.second()
        if time_op.is_same_day(now, self.m_now):
            return False
        return  now >= (time_op.get_today_start(now) + self.m_args[0])

    def __week_can_trigger(self):
        now = time_op.second()
        if time_op.is_same_week(now, self.m_now):
            return False
        if time_op.get_week_day(now) != self.m_args[0]:
            return False
        return now >= (time_op.get_today_start(now) + self.m_args[1])
